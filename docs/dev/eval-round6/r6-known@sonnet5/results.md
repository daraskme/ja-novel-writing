...
- 作成: 2026-09-19T13:47:15 / スキルのコミット: d7409ec（未コミットの変更: あり）
...
- ログの点検（スキルなしの系統と判定役のログに、スキルや結果のファイル名が出ていないか）: 9 本を点検して該当なし

## 対ごとの結果

| 依頼（ハッシュ） | 試行 | 1 回目（A / B → 選好） | 2 回目（A / B → 選好） | 結果 | 字数 with / base | 字数条件 with / base |
|---|---|---|---|---|---|---|
| graduation-confession（afba44ea0cb1） | 1 | with / base → with | base / with → base | **順序不安定** | 1270 / 1192 | 範囲内 / 不足 |
| romance-direct-confession（c0b6c036f4d2） | 1 | with / base → with | base / with → base | **順序不安定** | 587 / 523 | 指定なし / 指定なし |
| web-episode-twelve-reminder（af41771f1f46） | 1 | base / with → base | with / base → base | **base** | 2824 / 1226 | 指定なし / 指定なし |

## 依頼ごとの内訳

試行数が依頼ごとに違うので、合計の勝敗や勝率は出さない。過去のラウンドの結果とも合算しない。

- graduation-confession: 順序不安定 1
- romance-direct-confession: 順序不安定 1
- web-episode-twelve-reminder: base 1
...
|---|---|---|---|---|---|---|
| graduation-confession | 1 | with | 0 | 0 | 1 / 1（M02） | 4 / 7（C07 K00 L01 R01） |
| graduation-confession | 1 | base | 0 | 0 | 1 / 1（N06） | 5 / 8（K00 N05 O02 R01 X01） |
| romance-direct-confession | 1 | with | 0 | 0 | 1 / 1（M02） | 2 / 4（C07 K00） |
| romance-direct-confession | 1 | base | 0 | 0 | 1 / 1（N06） | 3 / 8（K00 L01 N05） |
| web-episode-twelve-reminder | 1 | with | 0 | 0 | 0 | 3 / 2（C09 L01 L04） |
| web-episode-twelve-reminder | 1 | base | 0 | 0 | 0 | 4 / 8（K00 L01 L02 N05） |
