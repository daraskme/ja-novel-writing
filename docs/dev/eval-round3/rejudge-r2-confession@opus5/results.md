...
- 作成: 2026-09-19T08:28:47 / スキルのコミット: 0561228（未コミットの変更: なし）
- 生成: `codex exec -s read-only --skip-git-repo-check -c model_reasoning_effort="medium" -C {cwd} -o {out} -`（codex-cli 0.155.1）
- 判定: `python -X utf8 evals/gateway.py chat --model anthropic/claude-opus-5 --timeout 600`（Python 3.13.15）。同じ対を A/B を入れ替えて 2 回読ませた。2 回で選好が変わった対は「順序不安定」
...
|---|---|---|---|---|---|---|
| graduation-confession（afba44ea0cb1） | 1 | base / with → with | with / base → base | **順序不安定** | 1376 / 1518 | 範囲内 / 超過 |
| graduation-confession（afba44ea0cb1） | 2 | with / base → with | base / with → with | **with** | 1339 / 1515 | 範囲内 / 超過 |

## 依頼ごとの内訳

試行数が依頼ごとに違うので、合計の勝敗や勝率は出さない。過去のラウンドの結果とも合算しない。

- graduation-confession: 順序不安定 1、with 1
...
|---|---|---|---|---|---|---|
| graduation-confession | 1 | with | 0 | 0 | 0 | 3 / 10（K00 L02 O02） |
| graduation-confession | 1 | base | 0 | 0 | 1 / 0（R03） | 4 / 8（K00 L01 L02 O02） |
| graduation-confession | 2 | with | 0 | 0 | 1 / 0（L01） | 2 / 5（K00 L02） |
| graduation-confession | 2 | base | 0 | 0 | 1 / 0（R03） | 4 / 11（K00 L01 L02 R01） |
