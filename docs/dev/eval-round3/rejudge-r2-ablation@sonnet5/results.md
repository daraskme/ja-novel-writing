...
|---|---|---|---|---|---|---|
| graduation-confession（afba44ea0cb1） | 1 | ablated / current → current | current / ablated → current | **current** | 1339 / 1351 | 範囲内 / 範囲内 |
| graduation-confession（afba44ea0cb1） | 2 | current / ablated → ablated | ablated / current → current | **順序不安定** | 1332 / 1333 | 範囲内 / 範囲内 |

## 依頼ごとの内訳

試行数が依頼ごとに違うので、合計の勝敗や勝率は出さない。過去のラウンドの結果とも合算しない。

- graduation-confession: current 1、順序不安定 1
