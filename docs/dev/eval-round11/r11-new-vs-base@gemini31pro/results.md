...
|---|---|---|---|---|---|---|
| romance-rooftop-mutual（492301c4fab0） | 1 | base / new → new | new / base → new | **new** | 1086 / 1260 | 範囲内 / 範囲内 |
| romance-festival-childhood-friend（d9379a240ab8） | 1 | base / new → base | new / base → new | **順序不安定** | 1267 / 855 | 指定なし / 指定なし |
| romance-restrained-reunion（350120861171） | 1 | base / new → new | new / base → new | **new** | 943 / 510 | 指定なし / 指定なし |
| romance-unrequited-farewell（b91872a754ab） | 1 | new / base → new | base / new → new | **new** | 1201 / 1225 | 指定なし / 指定なし |
| office-kitchenette-comedy（f6a3f2c02f27） | 1 | new / base → new | base / new → base | **順序不安定** | 947 / 987 | 範囲内 / 範囲内 |
| fantasy-inn-dinner（5bbca1863e63） | 1 | new / base → new | base / new → new | **new** | 1067 / 1062 | 指定なし / 指定なし |

## 依頼ごとの内訳

試行数が依頼ごとに違うので、合計の勝敗や勝率は出さない。過去のラウンドの結果とも合算しない。

- romance-rooftop-mutual: new 1
- romance-festival-childhood-friend: 順序不安定 1
- romance-restrained-reunion: new 1
- romance-unrequited-farewell: new 1
- office-kitchenette-comedy: 順序不安定 1
