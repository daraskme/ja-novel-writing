...
- ログの点検（スキルなしの系統と判定役のログに、スキルや結果のファイル名が出ていないか）: 18 本を点検して該当なし

## 対ごとの結果

| 依頼（ハッシュ） | 試行 | 1 回目（A / B → 選好） | 2 回目（A / B → 選好） | 結果 | 字数 new / base | 字数条件 new / base |
|---|---|---|---|---|---|---|
| romance-rooftop-mutual（492301c4fab0） | 1 | base / new → base | new / base → base | **base** | 1086 / 1260 | 範囲内 / 範囲内 |
| romance-festival-childhood-friend（d9379a240ab8） | 1 | base / new → base | new / base → base | **base** | 1267 / 855 | 指定なし / 指定なし |
| romance-restrained-reunion（350120861171） | 1 | base / new → base | new / base → base | **base** | 943 / 510 | 指定なし / 指定なし |
| romance-unrequited-farewell（b91872a754ab） | 1 | new / base → base | base / new → new | **順序不安定** | 1201 / 1225 | 指定なし / 指定なし |
| office-kitchenette-comedy（f6a3f2c02f27） | 1 | new / base → base | base / new → base | **base** | 947 / 987 | 範囲内 / 範囲内 |
| fantasy-inn-dinner（5bbca1863e63） | 1 | new / base → new | base / new → base | **順序不安定** | 1067 / 1062 | 指定なし / 指定なし |

## 依頼ごとの内訳

試行数が依頼ごとに違うので、合計の勝敗や勝率は出さない。過去のラウンドの結果とも合算しない。

- romance-rooftop-mutual: base 1
- romance-festival-childhood-friend: base 1
- romance-restrained-reunion: base 1
- romance-unrequited-farewell: 順序不安定 1
- office-kitchenette-comedy: base 1
- fantasy-inn-dinner: 順序不安定 1
...
| romance-rooftop-mutual | 1 | base | 0 | 0 | 1 / 2（N06） | 2 / 10（K00 N05） |
| romance-festival-childhood-friend | 1 | new | 0 | 0 | 1 / 1（M02） | 4 / 7（C07 D03 K00 O02） |
| romance-festival-childhood-friend | 1 | base | 0 | 0 | 2 / 9（D02 N06） | 3 / 4（D03 K00 N05） |
| romance-restrained-reunion | 1 | new | 0 | 0 | 1 / 1（M02） | 4 / 5（C07 K00 L01 R01） |
| romance-restrained-reunion | 1 | base | 0 | 0 | 1 / 2（N06） | 4 / 4（K00 L01 L02 N05） |
| romance-unrequited-farewell | 1 | new | 0 | 0 | 1 / 1（M02） | 5 / 8（C07 K00 L01 R01 T01） |
| romance-unrequited-farewell | 1 | base | 0 | 0 | 2 / 2（N06 R03） | 4 / 13（K00 L01 N05 R01） |
| office-kitchenette-comedy | 1 | new | 0 | 0 | 1 / 1（M02） | 3 / 7（C07 K00 R01） |
| office-kitchenette-comedy | 1 | base | 0 | 1 / 0（R02） | 2 / 2（R01 R03） | 3 / 7（K00 N05 R01） |
| fantasy-inn-dinner | 1 | new | 0 | 0 | 1 / 1（M02） | 5 / 7（C07 C09 K00 L01 O02） |
| fantasy-inn-dinner | 1 | base | 0 | 0 | 1 / 1（N06） | 3 / 4（K00 N05 R01） |
