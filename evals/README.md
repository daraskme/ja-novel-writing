...
結果の置き場所（`--run`）は、このリポジトリの外にする。過去の結果を、生成役に渡すスキルの写しへ含めないためである。
...
- スキルの系統には `SKILL.md`・`references/`・`scripts/`・`assets/` だけを写して渡す。`evals/` と `docs/`（過去の評価、敗因、修正の意図）は渡さない。スキルなしの系統と判定役は空のディレクトリで動かす。作業ディレクトリはリポジトリの外に、系統ごとに別の場所へ作り、名前から系統を推測できないようにする。`--run` は、比べるどの版のスキルの中にも置けない。
- 応答と判定は、作った条件のハッシュに結び付けて保存する（応答: 依頼文・系統の写し・生成コマンド・本文。判定: 依頼文・両応答・提示順・判定コマンド・判定の本文）。generate・judge・report のどれも同じ検証を通し、条件や本文が記録と違うものは使い回さず、読ませず、集計しない。終了コードが 0 でない判定と、最終行から選好を読めない判定は、選好が書かれていても失敗として記録する。引用（`>`）やコードブロックの中の「選好: A」は読まない。
...
- 作業ディレクトリと依頼本文は分けているが、ファイルの読み取りの隔離は保証していない。生成役にも判定役にも、他の系統、元のリポジトリ、結果・割り当て・ログ、利用環境の設定へアクセスできる可能性が残る。由来の情報を依頼文に直接含めない比較であり、厳密な盲検を保証するものではない。report は、スキルなしの系統と判定役のログにスキルや結果のファイル名が出ていないかを点検して `results.md` に書くが、これは事後の検知であって遮断ではない。ログが無ければ「該当なし」ではなく「確認不能」と書く。厳密にやるなら、役ごとに読み取り範囲を制限した実行環境（別ユーザー、コンテナ）で走らせる。
